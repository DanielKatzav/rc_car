...
// Register bits definitions (Continued)
#define nRF24_CONFIG_CRCO          (uint8_t)0x08 // CRC encoding scheme 0
#define nRF24_CONFIG_EN_CRC        (uint8_t)0x04 // Enable CRC
#define nRF24_CONFIG_MASK_RX_DR    (uint8_t)0x40 // Mask interrupt caused by RX_DR
#define nRF24_CONFIG_MASK_TX_DS    (uint8_t)0x20 // Mask interrupt caused by TX_DS
#define nRF24_CONFIG_MASK_MAX_RT   (uint8_t)0x10 // Mask interrupt caused by MAX_RT
#define nRF24_RF_SETUP_CONT_WAVE   (uint8_t)0x80 // Continuous carrier transmit
#define nRF24_RF_SETUP_RF_DR_LOW   (uint8_t)0x20 // Set RF Data Rate to 250 kbps
#define nRF24_RF_SETUP_RF_DR_HIGH  (uint8_t)0x08 // Set RF Data Rate to 2 Mbps
#define nRF24_RF_SETUP_PLL_LOCK    (uint8_t)0x10 // Force PLL lock signal
#define nRF24_STATUS_RX_DR         (uint8_t)0x40 // Data Ready RX FIFO interrupt
#define nRF24_STATUS_TX_DS         (uint8_t)0x20 // Data Sent TX FIFO interrupt
#define nRF24_STATUS_MAX_RT        (uint8_t)0x10 // Maximum number of TX retransmits interrupt
#define nRF24_RX_PW_P0_DEFAULT     (uint8_t)0x00 // Default size of RX payload in data pipe 0
#define nRF24_RX_PW_P1_DEFAULT     (uint8_t)0x00 // Default size of RX payload in data pipe 1
// ... (Additional register bits definitions)

// Register masks definitions (Continued)
#define nRF24_MASK_R_RX_PL_WID     (uint8_t)0x3F // Mask bits [5:0] in RX_PW_Px registers
// ... (Additional masks definitions)
